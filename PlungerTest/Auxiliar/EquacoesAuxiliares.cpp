/**
 * @file EquacoesAuxiliares.cpp
 * @author Equipe AUTOPOC Plunger Lift
 *  Implementação da interface para o modelo matemático da simulação.
 * 	Revisões:
 *
 */
//---------------------------------------------------------------------------
...
#include "SimulatorPoint.h"
//---------------------------------------------------------------------------
#include<stdlib.h>
// Indica que a instância está nula
EquacoesAuxiliares* EquacoesAuxiliares :: instance = NULL ;
// Indica que a instância ainda não foi iniciada
bool EquacoesAuxiliares :: instanceFlag = false ;
/**
 ...
 */
EquacoesAuxiliares* EquacoesAuxiliares :: getInstance() {
		if ( !instanceFlag ){
			EquacoesAuxiliares :: instance = new EquacoesAuxiliares();
		}
		return EquacoesAuxiliares :: instance;
}
//---------------------------------------------------------------------------
/**
 ...
 */
EquacoesAuxiliares::EquacoesAuxiliares() {
	//Configuracoes iniciais do algoritmo
	EquacoesAuxiliares::instanceFlag = true;
	tempo             = 0;
	idRamoSimulacao   = 0;
	quantidadePontos  = 0;
	bufferSendPoints  = 6;
	periodoAmostragem = 200;
...
//---------------------------------------------------------------------------
/**
 * @brief Resetar a classe.
 */
void EquacoesAuxiliares::Limpar() {
...
//---------------------------------------------------------------------------
/**
 * @brief Inicia os parâmetros para poder começar a simulação.
 */
void EquacoesAuxiliares::iniciarSimulacao() {
	...
	c->Ppc = (677 + 15*f->fluido->SGgas - 37.5*powl(f->fluido->SGgas,2)) *
																														 6.894757*pow(10,3);
	c->Tpc = ((168 + 325*f->fluido->SGgas - 12.5*pow(f->fluido->SGgas,2))-491.67)*
																																 5.0/9.0+273.15;

	//VAZAO MAXIMA DO RESERVATORIO
	c->Qmax = f->reservat->Qteste/(1-.2*f->reservat->Pteste/f->reservat->Pest -
															.8*powl(f->reservat->Pteste/f->reservat->Pest,2));
	...
	f->varSaida->PcsgB = EquacoesUtilitarias::GASOSTB(f->tempos->PcsgT, c->Tsup,
								EquacoesUtilitarias::TEMP(f->tubing->Lcauda),f->tubing->Lcauda);
	...
	v->PtbgB = f->varSaida->PcsgB -
				 ( c->ROliq * c->G * f->tempos->Lslg + f->pistao->Mplg * c->G/c->AItbg);

	//Pressao no topo do tubing pela equacao GASOSTT
	f->varSaida->PtbgT = EquacoesUtilitarias::GASOSTT(v->PtbgB,
														    EquacoesUtilitarias::TEMP(v->H), c->Tsup, v->H);
...
bool EquacoesAuxiliares::inicioCiclo(){
	...
	if ( v->Ntotal == 0 )
		...
		v->N = v->Ntotal = (v->PPcsg * c->Vcsg)/(EquacoesUtilitarias::Z(
													 v->PPcsg/c->Ppc, c->TTcsg/c->Tpc) * c->R * c->TTcsg);
	else
	...
	v->H = f->tubing->Lcauda - f->varSaida->Hplg - f->pistao->Lplg -
																																f->tempos->Lslg;
	...
	v->TT = (EquacoesUtilitarias::TEMP(v->H) + c->Tsup)/2.0;
	//Z Fator de compressibilidade médio
	v->z = EquacoesUtilitarias::Z(v->PP/c->Ppc, v->TT/c->Tpc);
	...
	v->q = EquacoesUtilitarias::QSC(f->varSaida->PtbgT/1000.0,
														f->linhaPro->Psep/1000.0, f->valvula->Dab, c->Tsup);
	...
	} while ( fabs(v->PP - f->varSaida->pp) > 1.0 );

	//Inicializa pressão no topo do tubing descontando a influência do pistão
	f->varSaida->PtbgT = ( f->varSaida->pp * 2 ) / ( 1 + expl( ( c->PM * c->G *
	...
	do {
		...
		v->qq = (v->q + EquacoesUtilitarias::QSC(v->Pt/1000.0,
		...
		f->varSaida->PtbgT = (f->varSaida->pp * 2)/(1 + expl((c->PM * c->G *
							(f->tubing->Lcauda - f->tempos->Lslg - f->varSaida->Hplg))/
							(v->z * c->R * v->TT)));
	} while( fabs(f->varSaida->PtbgT - v->Pt) > 1.0 && v->y++ < 50);
	...
	v->fatorB = v->PtbgB-EquacoesUtilitarias::GASOSTB(f->linhaPro->Psep,
																c->Tsup, EquacoesUtilitarias::TEMP(v->H), v->H);
	...
	v->I = ( powl( f->tubing->DItbg,2 )*pow( f->fluido->APi,0.5 )*1000 /
										powl( f->tempos->Lslg + 100,0.3 ) )*( 1 - f->pistao->EfVed );
...
void EquacoesAuxiliares::subidaPistao(){
	...
	for( v->d = 0, v->i = 1; (v->H - v->delta_h) > 0 &&
		...
		v->qqq = EquacoesUtilitarias::QSC(f->varSaida->PtbgT/1000.0,
														f->linhaPro->Psep/1000.0, f->valvula->Dab, c->Tsup);
		...
		do{
			...
			v->q = EquacoesUtilitarias::QSC(f->varSaida->PtbgT/1000.0,f->linhaPro->Psep/1000.0,f->valvula->Dab,c->Tsup);
			if(v->q == 0.0) {
				break;
			}
		...
		}while( fabs( v->Ptt - f->varSaida->PtbgT) > 1.0 && v->y++ < 50);
		//CALCULA A PRESSÃO NA BASE DA COLUNA
		v->PtbgB = (f->varSaida->pp * 2)/(1 + expl(- (c->PM * c->G * (f->tubing->Lcauda - f->tempos->Lslg - f->varSaida->Hplg))/(v->z * c->R * v->TT)) );
		...
		v->fatorB = v->PtbgB - EquacoesUtilitarias::GASOSTB(f->linhaPro->Psep, c->Tsup, EquacoesUtilitarias::TEMP(v->H),v->H);
		...
		if(f->varSaida->Hplg >= 0){
			...
			if( v->i == 1 ){
				...
				v->PplgM = EquacoesUtilitarias::GASOSTT(f->varSaida->PcsgB - c->ROliq * c->G * f->tempos->Ltbg, EquacoesUtilitarias::TEMP(f->tubing->Lcauda - f->tempos->Ltbg), EquacoesUtilitarias::TEMP(f->tubing->Lcauda - f->varSaida->Hplg), f->varSaida->Hplg - f->tempos->Ltbg);
				...
				v->Visc   = EquacoesUtilitarias::VISC ( EquacoesUtilitarias::TEMP( f->tubing->Lcauda - f->varSaida->Hplg - f->tempos->Lslg/2 ) );
				//CALCULA O Nº DE REYNOLDS
				v->Rey    = c->ROliq * fabs(v->v0) * f->tubing->DItbg/v->Visc;
				//CALCULA O FATOR DE FRICÇÃO
				v->Fric   = EquacoesUtilitarias::FRIC(v->Rey, f->tubing->E, f->tubing->DItbg);
				//CALCULA A PERDA POR FRICÇÃO
				v->Pfric  = c->ROliq * v->Fric * f->tempos->Lslg * fabs(v->v0) * v->v0/(2 * f->tubing->DItbg);
			}
			...
			f->varSaida->Qlres = c->Qmax * (1 - .2*(v->Pwf/f->reservat->Pest) - .8*powl(v->Pwf/f->reservat->Pest, 2));
			...
			if( v->Vt <= 0 ) {
				...
				do{
					...
					v->Pbt=(v->p_*2)/(1+expl(-(c->PM*c->G*(f->varSaida->Hplg-f->tempos->Ltbg))/(v->z*c->R*v->TTt)));
					...
					v->Pba = (v->p_ * 2)/(1 + expl(-(c->PM * c->G * f->tubing->Lcauda)/(v->z * c->R * c->TTcsg)));
				...
				f->tempos->PcsgT = (v->p_*2)/(1+expl((c->PM*c->G*f->tubing->Lcauda)/(v->z*c->R*c->TTcsg)));
			}
...
void EquacoesAuxiliares::producaoLiquido(){
	...
	v->Pfric += v->delta_h * c->ROliq * v->v0 * (pow(f->tubing->DItbg, 2)/(f->valvula->Dab/1000, 2) - 1)/c->step; //CUIDADO! , SEM EFEITO
	...
	for(v->j = 0; f->tubing->Lcauda > f->varSaida->Hplg &&
		...
		v->PplgM   = EquacoesUtilitarias::GASOSTT(v->PtbgB, EquacoesUtilitarias::TEMP(f->tubing->Lcauda - f->tempos->Ltbg),EquacoesUtilitarias::TEMP(f->tubing->Lcauda - f->varSaida->Hplg), f->varSaida->Hplg - f->tempos->Ltbg);
		...
		v->PplgM  -= (v->Pfric + c->ROliq * powl(v->v0*powl(f->tubing->DItbg,2) / powl(f->valvula->Dab/1000,2),2) / 2.0);
		...
		double Visc = EquacoesUtilitarias::VISC(EquacoesUtilitarias::TEMP(f->tempos->Ltbg));
		//COEFICIENTE DE AMORTECIMENTO VISCOSO
		double amort = (Visc * 3 * M_PI * pow(f->pistao->Dplg,3) * f->tempos->Ltbg) * (1 + ((f->tubing->DItbg - f->pistao->Dplg) / f->pistao->Dplg)) / (4*powl((f->tubing->DItbg - f->pistao->Dplg) / 2,3));
		...
		v->Visc   = EquacoesUtilitarias::VISC(EquacoesUtilitarias::TEMP(( f->tempos->Lslg/2.0 < 0.0 ? 0.0 : f->tempos->Lslg/2.0)));
		//CALCULA O NUMERO DE REYNOLDS
		v->Rey    = c->ROliq * fabs(v->v0) * f->tubing->DItbg/v->Visc;
		//CALCULA O FATOR DE FRICCAO
		v->Fric   = EquacoesUtilitarias::FRIC(v->Rey, f->tubing->E, f->tubing->DItbg);
		...
		v->Pfric += v->delta_h*c->ROliq*v->v0*(pow(f->tubing->DItbg,2) /(f->valvula->Dab/1000,2)-1)/ c->step_;
		//CRIA UMA MENSAGEM PARA A INTERFACE ATUALIZAR SEUS ATRIBUTOS
		criarMensagem(PRODUCTION);
	...
	//STAGE_PRODUC_DURATION = 2
	enviarVarCiclo(2, v->j * c->step_);
...
void EquacoesAuxiliares::Controle(){
	...
	if( f->tubing->Lcauda <= f->varSaida->Hplg){
		...
		v->piston_arrival = v->j*c->step_ + v->i*c->step + v->transient;
	}
	//SE O PISTAO NAO CHEGOU NA SUPERFICIE
	else{
		//TEMPO DE CHEGADA DO PISTAO E ZERO
		v->piston_arrival = 0.0;
	}

	//CALCULA A PRODUCAO
	v->production = (v->LslgX - f->tempos->Lslg)*c->AItbg*6.2848352758387*(1- c->FW);
  //PRODUÇÃO ACUMULADA
	...
	if ( !byPassController ) {
		...
		if( v->contador == 0){
			//INCREMENTA CP
			v->CP += v->CP*EquacoesUtilitarias::fuzzy_control(10*(c->good_velocity - v->velocity));
		}
	...
	ConstantesUniversais::getInstance()->estagio = ConstantesUniversais::getInstance()->CONTROLE;
...
void EquacoesAuxiliares::Afterflow(){
	...
	for(v->k = 1; v->k*c->step_aft < f->tempos->Afterflow && (!this->alterarValvula);
		...
		v->qqq = EquacoesUtilitarias::QSC( v->Pt/1000.0 , f->linhaPro->Psep/1000.0, f->valvula->Dab, c->Tsup);
		...
		do{
			...
			v->q=EquacoesUtilitarias::QSC(v->Pt/1000.0, f->linhaPro->Psep/1000.0, f->valvula->Dab, c->Tsup);
			...
			v->Pt= EquacoesUtilitarias::GASOSTT(v->PtbgB/*v->Pba - c->ROliq * c->G * f->tempos->Ltbg*/, EquacoesUtilitarias::TEMP(f->tubing->Lcauda - f->tempos->Ltbg), c->Tsup, f->tubing->Lcauda - f->tempos->Ltbg);
		} while( fabs(v->Ptt - v->Pt) > 1.0 );   /* fim do DO */
	...
	//STAGE_AFTER_DURATION = 8
	enviarVarCiclo(8, v->k * c->step_aft);

	//INFORMA QUE A ETAPA QUE ACABOU FOI O AFTERFLOW
	ConstantesUniversais::getInstance()->estagio =
																 ConstantesUniversais::getInstance()->AFTERFLOW;
}
//---------------------------------------------------------------------------
/**
...
//	v->limite = (f->tubing->Lcauda - f->tempos->Ltbg) * (60*3.2808)/ f->pistao->Vqpg + ((v->Ppart_csg - v->Ppart_tbg) * (60*3.2808)/(c->ROliq * c->G * f->pistao->Vqpl));
	...
	for( v->m = 1; ( tpgasto < f->tempos->Offtime ||
		...
		double DplgEsf = powl(pow(f->pistao->Dplg,2) * f->pistao->Lplg * 1.5, 0.3333);
		...
		if ( f->varSaida->Hplg > f->tempos->Ltbg ) {
			...
			double visgas = EquacoesUtilitarias::VISGAS(EquacoesUtilitarias::TEMP(f->tubing->Lcauda - f->tempos->Ltbg), c->ROgas);
			...
			if ((f->varSaida->Hplg - f->tempos->Ltbg)>(f->tubing->Lcauda - f->tempos->Ltbg)*0.15) {
				v->v0 = -(eqFgas/eqk)*(1 - expl(-eqk*eqt/f->pistao->Mplg));
			}
			else{
		...
		//Se pistão estiver no Líquido
			...
			double visliq = EquacoesUtilitarias::VISC(EquacoesUtilitarias::TEMP(f->tempos->Ltbg));
			...
			v->v0 = -(eqFliq/eqk)*(1 - expl(-eqk*eqt/f->pistao->Mplg));
			modo_passo = 2;
		}
	...
	//Calcula velocidade na interface
			...
			v->v0 = porcentagliq*(-(eqFliq/eqkL)*(1 - expl(-eqkL*eqt/f->pistao->Mplg))) +
							(1 - porcentagliq)*(-(eqFgas/eqkG)*(1 - expl(-eqkG*eqt/f->pistao->Mplg)));
		...
		v->TTt= (EquacoesUtilitarias::TEMP(f->tubing->Lcauda - v->temp) + c->Tsup)/2.0;
		//INICIALIZA CONTADOR
		v->y=0;
		...
		do{
			...
			for( v->u = 0; fabs(v->PPt - f->varSaida->pp) > 1.0 && v->u < 20; v->u++ ) {
			...
			v->Pbt = ( f->varSaida->pp * 2)/(1 + expl(-(c->PM * c->G * (f->tubing->Lcauda - v->temp))/(v->z * c->R * v->TTt)));
			...
			for(v->u = 0; fabs(v->PPcsg - f->varSaida->pp) > 1.0 && v->u<20; v->u++){
				//ATUALIZA A PRESSÃO MÉDIA DO REVESTIMENTO
			...
			//CALCULA A PRESSÃO NA BASE DO ESPAÇO ANULAR
			v->Pba = (f->varSaida->pp*2)/(1 + expl(-(c->PM * c->G * f->tubing->Lcauda)/(v->z * c->R * c->TTcsg)));
		...
		//CALCULA A PRESSÃO NO TOPO DO REVESTIMENTO
		f->tempos->PcsgT = (v->save_PPcsg * 2)/(1 + expl((c->PM * c->G * (f->tubing->Lcauda))/(v->z * c->R * c->TTcsg)));
		...
		f->varSaida->PtbgT = EquacoesUtilitarias::GASOSTT(v->PtbgB,EquacoesUtilitarias::TEMP(f->tubing->Lcauda - v->temp), c->Tsup, f->tubing->Lcauda - v->temp );
	...
	ConstantesUniversais::getInstance()->estagio =
		ConstantesUniversais::getInstance()->OFF_BUILD_UP;
	...
	// + BUILDUP)
	enviarFimCiclo( v->i * c->step +  v->j * c->step_ + v->k * c->step +
										v->m * c->_step );
}
//---------------------------------------------------------------------------
/**
...
void EquacoesAuxiliares::criarMensagem(CycleStage stage){
	...
	switch (stage) {
		case PLUNGER_RISE:
			this->tempo = this->tempo + c->step ;
			break;
		case PRODUCTION:
			this->tempo = this->tempo + c->step_;
			break;
		case AFTERFLOW:
			this->tempo = this->tempo + c->step_aft;
			break;
		case BUILDUP:
			this->tempo = this->tempo + c->_step;
			break;
	}

	//ADICIONA A CONTAGEM DE QUANTIDADES DE PONTOS
	quantidadePontos = quantidadePontos + 1;
	//cout << "Pontos - " << quantidadePontos << "\n";
	...
	if ( quantidadePontos > periodoAmostragem ) {
		...
		cout << "Altura do Pistao: " << f->varSaida->Hplg << "\n";
	}
 ...
 */
void EquacoesAuxiliares::enviarVarCiclo(int ciclovar, double valor) {
//TODO
}
/**
